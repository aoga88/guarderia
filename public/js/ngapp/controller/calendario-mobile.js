function CalendarioMobileController($scope, Registro, Alumno) {
	...
	{
		var hoy = new Date();
		var fechaStr = hoy.getDate() + '/' + (hoy.getMonth() + 1) + '/' + hoy.getFullYear();
		if (fechaStr === info.fecha)
		{
			return true;
		} else {
			return false;
	...
	$scope.setActualFecha = function(dia, info) {
		$scope.actualFecha = {
			dia: dia,
			info: info
		};
